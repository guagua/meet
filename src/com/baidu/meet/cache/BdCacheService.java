...
import com.baidu.meet.cache.BdKVCache.MXSupportedCache;
 ...
 * 
 * <p><b>使用技巧：</b>
 * <br/>1. 表分裂后，也就是使用 {@link CacheStorage#SQLite_CACHE_PER_TABLE} 进行cache清空时，
 *         性能比  {@link CacheStorage#SQLite_CACHE_All_IN_ONE_TABLE} 快40%左右。其他性能类似，查询时All_IN_ONE略高。
 * <br/>
 * <br/>2. 对于频繁有数据淘汰的表，也就是LRU算法cache或是手工调用remove操作的表。使用 {@link #newInstance(String)} 将cache分配到单独的数据库中，可以获得更好的性能。
 *         SQLite的删除性能与表大小关系不大，主要是库大小，也就是要删除的记录后面有多少记录。后面的记录越多，性能越差。分配到单独的数据库将会分配单独的存储文件，存储文件越小，性能越高。
 * </p>
 * 
...
public class BdCacheService {
	...
	private BdCacheService(String databaseFile){
		...
		if(BaseApplication.getApplication() != null){
			this.debugMode = BaseApplication.getApplication().isDebugMode() ;
		}
	}
	...
	public synchronized String initAndGetTableName(BdCacheBaseDBManager<?> dbManager, String nameSpace, String cacheType, int maxSize){
		...
		if(item == null){
			...
			if(!cacheType.equalsIgnoreCase(item.cacheType)){
				throw new IllegalArgumentException("nameSpace [" + nameSpace + "] is already taken by cacheType:" + item.cacheType) ;
			}
	 ...
	 * 
	 * <p>
	 * <b>性能：</b> 在ZET普通手机上测试，第一次创建DB时间为146ms，每个Cache第一次初始化创建表的时间为2ms到17ms；
	 * <br/> 在小米2S上测试，第一次数据库创建时间为0ms，每个Cache第一次初始化创建表的时间为10ms到60ms。
	 * <br/> 完成第一次表创建后，后续创建时间为0ms【不包括LRU策略预加载或是淘汰数据的时间--异步线程】。
	 * </p>
	 * 
	 * @param nameSpace 命名空间，缓存内容清理策略都是基于命名空间进行。不同命名空间之间的key可以重复。最多128个字符。
	 * @param storage 存储策略
	 * @param policy 淘汰策略
	 * @param maxSize 最大保留的cache条数
	 * 
	 * @see #returnAndStopTextCache(BdKVCache)
	 */
	public synchronized BdKVCache<String> getAndStartTextCache(String nameSpace, CacheStorage storage, CacheEvictPolicy evictPolicy, int maxSize) {
		...
		//Shared Database Connection is used in BdTextCachePerTableDBManager, so BdTextCachePerTableDBManager can be created on fly.
		...
		if(m_old != null){
			if(storageImpl != null && m_old instanceof BdKVCache.MXSupportedCache){
				if(((BdKVCache.MXSupportedCache<String>) m_old).getCacheStorage() != storageImpl){
					throw new IllegalStateException("nameSpace:[" + nameSpace + "] is already used for storage:[" + storageImpl + "]. Make sure to return the old cache before re-use the same namespace.") ;
				}
			}
	 ...
	 * 
	 * <p>
	 * <b>性能：</b> 在小米2S上测试，第一次数据库创建时间为0ms，每个Cache第一次初始化创建表的时间为16ms到28ms。
	 * <br/> 完成第一次表创建后，后续创建时间为0ms【不包括LRU策略预加载或是淘汰数据的时间--异步线程】。
	 * </p>
	 * 
	 * @param nameSpace 命名空间，缓存内容清理策略都是基于命名空间进行。不同命名空间之间的key可以重复。最多128个字符。
	 ...
	 * @see #returnAndStopTextCache(BdKVCache)
	 */
	public synchronized BdKVCache<byte[]> getAndStartBlobCache(String nameSpace, CacheStorage storage, CacheEvictPolicy evictPolicy, int maxSize) {
		...
		if(cache instanceof BdKVCache.MXSupportedCache){
			...
			synchronized (mxCache) {
				...
				try{
					...
					MeetLog.e("failed to close cache:" + ns) ;
				}
			}
		}
	}
